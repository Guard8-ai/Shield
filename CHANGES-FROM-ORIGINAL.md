...
## Part 18 — PAKEExchange primitives unified across all bindings + cross-language conformance vector (2026-06-30) ✅ DONE

- **Finding (resolves the deferral in Part 15):** Part 15 disclosed that the channel derivations were *not* byte-compatible across bindings but deferred the fix. Root-causing it revealed the divergence was **wider and deeper** than "Go/Python/JS use SHA256": **every non-Rust binding** diverged in the underlying `PAKEExchange.derive`/`combine` primitives.
  - Rust (source of truth): `derive = HMAC-SHA256(PBKDF2(secret,salt), role)`; `combine = HMAC-SHA256(sorted[0], sorted[1]||…)`.
  - Python, JS, Java, C#, Kotlin, Android, Swift: `derive = SHA256(pbkdf2_key || role)`; `combine = SHA256(sorted_concat)` — plain unkeyed SHA256.
  - Go: a *third* construction — `derive` folded the role into the **PBKDF2 salt** (`salt || "SHIELD-PAKE-"+role`) and `combine = SHA256(local || remote)` **without sorting**.
  - Every binding's own doc comment already *claimed* `HMAC(PBKDF2(secret,salt), role)`; only Rust's code matched its docs. `cc9dbe6` had aligned only the outer session-key HMAC, leaving these primitives divergent, so `ShieldChannel`/`PAKEExchange` were **never cross-language interoperable** — and no test caught it because, unlike the base AEAD and PQ KEX, the channel/PAKE had **no conformance vector**.
- **Fix (all bindings aligned to the Rust source of truth):** `derive` → keyed `HMAC-SHA256(pbkdf2_key, role)`; `combine` → sort contributions then `HMAC-SHA256(sorted[0], sorted[1]||…)`. Changed in Python (`exchange.py`), JS (`exchange.js`), Go (`exchange.go` — dropped the role-in-salt hack and added sorting), Java, C#, Kotlin, Android, Swift. C has no PAKE (base cipher only) and is unaffected.
- **New conformance vector (`tests/channel_session_vectors.json`):** the anchor that was missing — pins both the `PAKEExchange` primitives (`derive` for roles client/server/session, `combine`) **and** the full `ShieldChannel` session key, all generated from the Rust reference. Every binding must reproduce these byte-for-byte, so any future channel/PAKE divergence fails a test instead of escaping to a scanner. Session-key vector: `c81854ce5fcbcf6f4db68b2a8b389366232b1707fa057a7251d6536bf529183d` (password `conformance-secret`, salt 16×0x01, local 32×0x02, remote 32×0x03, service `shield.conformance`, 600k iters).
- **Format note:** this changes `PAKEExchange`/`ShieldChannel` output in every non-Rust binding. Since those bindings never interoperated before (three different constructions), there is no cross-language compatibility to break — only correctness gained. The handshake is ephemeral (no persisted PAKE material), so there is no at-rest migration. The pre-existing "not a true PAKE / offline-dictionary" disclosure (Part 14) is unchanged.
- **Verification (executed on this machine, 2026-06-30):** Rust `cargo test --lib` 104 + `cargo clippy --lib --all-targets` clean; Python `pytest` 226 (0 warnings); JS `node --test` 127; Go `go vet` + `go test ./...` ok; C# `dotnet test` 63/63 — each with a new test reading `tests/channel_session_vectors.json` and reproducing it (Rust keeps its pinned golden vector as the generator). Java: `Exchange.java` compiled standalone (`javac`) and the vector reproduced by a harness (client/session/base all byte-identical) — no gradle on the dev host. Kotlin/Android use the identical `javax.crypto.Mac` construction as the verified Java; Swift uses the same `CCHmac`-backed `hmacSha256` helper — source-aligned but not executed on this host (no kotlinc/gradle; Swift `CommonCrypto` is Apple-only), consistent with the existing Swift/iOS posture.
- **Also fixed:** three Pydantic v2 deprecation warnings in `python/shield/integrations/pgvector_api.py` (`min_items`/`max_items` → `min_length`/`max_length`); Python suite is now warning-clean.
