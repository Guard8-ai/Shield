...
class StreamCipherEofTest {
    ...
    @Test
    fun `eof tag conformance vector`() {
    ...
    fun `stream roundtrip`() {
        ...
        StreamCipher.create(key, 16).use { sc ->
            ...
            assertContentEquals(data, sc.decrypt(sc.encrypt(data)))
        }
    }

    @Test
    fun `truncation at chunk boundary rejected`() {
        val key = ByteArray(32) { 0x42 }
        StreamCipher.create(key, 16).use { sc ->
            val data = ByteArray(64) { it.toByte() }
            val enc = sc.encrypt(data)
            val truncated = enc.copyOf(enc.size - 36) // drop marker + tag
            assertThrows<Exception> { sc.decrypt(truncated) }
        }
    }

    @Test
    fun `forged end marker rejected`() {
        val key = ByteArray(32) { 0x42 }
        StreamCipher.create(key, 16).use { sc ->
            val data = ByteArray(64) { it.toByte() }
            val enc = sc.encrypt(data)
            val forged = enc.copyOf(enc.size - 36 + 4) // bare zero marker, no tag
            assertThrows<Exception> { sc.decrypt(forged) }
        }
    }
}
