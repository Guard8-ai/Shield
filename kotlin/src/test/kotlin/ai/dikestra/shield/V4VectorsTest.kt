...
class V4VectorsTest {
    ...
    private fun masterFor(obj: String): ByteArray {
        return if (str(obj, "mode") == "password") {
            Shield.createWithSalt(
                str(obj, "password")!!, str(obj, "service")!!,
                hex(str(obj, "salt_hex")!!), num(obj, "iterations").toInt(), null
            ).getKey()
        } else {
            hex(str(obj, "key_hex")!!)
        }
    ...
    fun testKdfVectors() {
        ...
        for (v in vectors) {
            ...
            assertEquals(str(v, "aead_key_hex"), toHex(Shield.deriveAeadKey(master)), "aead drift ${str(v, "name")}")
        }
    }
