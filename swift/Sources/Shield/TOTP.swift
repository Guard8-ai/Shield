...
public class RecoveryCodes {
    ...
    public init(count: Int = 10) {
        ...
        for _ in 0..<count {
            if let code = RecoveryCodes.generateCode() {
                generatedCodes.insert(code)
            }
        }
        self.codes = generatedCodes
    }

    private static func generateCode() -> String? {
        guard let bytes = Shield.randomBytes(4) else { return nil }
        let part1 = (Int(bytes[0]) << 8) | Int(bytes[1])
        let part2 = (Int(bytes[2]) << 8) | Int(bytes[3])
    ...
    public func verify(_ code: String) -> Bool {
        ...
        let formatted = normalized.count == 8 ? "\(normalized.prefix(4))-\(normalized.suffix(4))" : normalized
