...
/**
 * Conformance: the Android post-quantum hybrid KEX must satisfy the shared
 ...
 * Runs on the local JVM (testDebugUnitTest), so Bouncy Castle is available.
 */
class PqKexVectorsTest {
    ...
    fun reproducesAllVectors() {
        ...
        assertTrue("no vectors loaded", privs.isNotEmpty())

        for (i in privs.indices) {
            val bob = HybridPrivateKey.fromBytes(hex(privs[i]))
            assertEquals("bundle mismatch for ${names[i]}", bundles[i], hx(bob.publicKey().toBytes()))
            val shared = bob.accept(hex(handshakes[i]))
            assertEquals("shared key mismatch for ${names[i]}", shareds[i], hx(shared))
        }
    }
    ...
    private fun assertThrows(expected: Class<out Throwable>, block: () -> Unit) {
        try {
            ...
            throw AssertionError("expected ${expected.name} but got ${t::class.java.name}")
        }
