...
mod tests {
    ...
    ///
    /// Uses a fixed nonce and root_key so the output is reproducible.
    /// Run with: cargo test ratchet_vector -- --nocapture
    ///
    /// The printed JSON should match tests/ratchet_vectors_family_a.json.
    #[test]
    fn test_ratchet_vector() {
        let root = [0x42u8; 32];

        // Reproduce intermediate chain values
        let send_chain = derive_chain_key(&root, b"send");
        let (new_send_chain, msg_key) = ratchet_chain(&send_chain);
        let (enc_key, mac_key) = derive_msg_subkeys(&msg_key);
        ...
        hmac_data.extend_from_slice(&nonce);
        hmac_data.extend_from_slice(&ciphertext);
        let mac_tag = hmac::sign(&hmac_mac_key, &hmac_data);

        let mut frame = Vec::new();
        frame.extend_from_slice(&nonce);
        frame.extend_from_slice(&ciphertext);
        frame.extend_from_slice(&mac_tag.as_ref()[..16]);

        // Print JSON vector for cross-language consumption
        println!(
            r#"{{
  "schema_version": "1",
  "family": "A",
  "description": "Rust HMAC reference — generated by test_ratchet_vector",
  "root_key": "{}",
  "is_initiator": true,
  "counter": 0,
  "plaintext_hex": "{}",
  "plaintext_utf8": "Hello, Shield!",
  "fixed_nonce_hex": "{}",
  "intermediate": {{
    "send_chain_after_init": "{}",
    "msg_key": "{}",
    "new_send_chain": "{}",
    "enc_key": "{}",
    "mac_key": "{}"
  ...
  "frame_layout": "nonce(16) || HMAC_keystream_XOR(ctr_le64||plaintext) || hmac_sha256_truncated_16(16)"
            ...
            hex::encode(nonce),
            hex::encode(send_chain),
            hex::encode(msg_key),
            hex::encode(new_send_chain),
            hex::encode(enc_key),
            hex::encode(mac_key),
            hex::encode(&frame),
        );

        // Verify the vector matches the known-good value from ratchet_vectors_family_a.json
        let expected_ct = "000102030405060708090a0b0c0d0e0f0ef3fefe86d0a95843da2ae531ad4af11ba8a474aa86c024fdb7113afa2cb0d0f3cb423bd6a7";
        assert_eq!(
            hex::encode(&frame),
            expected_ct,
            "Rust ratchet vector does not match expected cross-language value"
        );
    }
}
