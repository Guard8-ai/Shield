...
fn test_key_derivation_matches_python() {
    // Derive key using same PBKDF2 parameters as Shield::new()
    let salt = ring::digest::digest(&ring::digest::SHA256, test_vectors::SERVICE.as_bytes());
    let mut key = [0u8; 32];
    ring::pbkdf2::derive(
        ring::pbkdf2::PBKDF2_HMAC_SHA256,
        NonZeroU32::new(100_000).unwrap(),
        salt.as_ref(),
        test_vectors::PASSWORD.as_bytes(),
        &mut key,
    );
